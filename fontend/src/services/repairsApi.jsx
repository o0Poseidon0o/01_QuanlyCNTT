...
const mapTicketFromApi = (row = {}) => ({
  ...
  severity: fromBackendSeverity(row.severity),
  priority: fromBackendPriority(row.priority),
});

const mapHistoryFromApi = (entry = {}) => ({
  ...entry,
  old_status: fromBackendStatus(entry.old_status),
  new_status: fromBackendStatus(entry.new_status),
});
...
const mapSummaryFromApi = (summary = {}) => {
  const status = Array.isArray(summary.status)
    ? summary.status.map((item) => ({
        ...item,
        name: fromBackendStatus(item.name),
      }))
    : [];
