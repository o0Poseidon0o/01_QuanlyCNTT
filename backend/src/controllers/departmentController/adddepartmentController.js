...
const addDepartment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Có lỗi xảy ra khi tạo phòng ban.", error });
  }
};
...
const searchDepartments = async (req, res) => {
  try {
    ...
    if (!name || name.trim() === "") {
      return res
        .status(400)
        .json({ message: "Vui lòng cung cấp tên phòng ban hoặc ID." });
    }

    // Kiểm tra xem `name` có phải là một số hay không
    const isIdSearch = !isNaN(name);
    const searchCriteria = isIdSearch
      ? { id_departments: Number(name) }
      : { department_name: { [Op.like]: `%${name}%` } };
...
const deleteDepartment = async (req, res) => {
  try {
    ...
    res.json({
      message:
        "Xóa phòng ban thành công! Các người dùng liên quan đã được cập nhật.",
    });
  } catch (error) {
